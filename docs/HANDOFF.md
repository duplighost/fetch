...
round-five plan, which was right about where to look and wrong about two of its
guesses — and the wrong guesses are where the real bugs were, so they are
...
- **§1 the centre stream ends in a pond.** The ribbon widens as it runs south
  and ends in standing water with a rock rim, in the SAME mesh and the same
  material as the stream: zero draw calls, zero geometries. Its rim stones are
  ankle height and, unlike the ones he complained about, solid — below STEP_UP
  a collider is a step you walk onto, not a wall.
- **§2 the six "concrete/brick" blocks are gone**, and the basin has a real
  lip. Their maths had lost `CLEARING_BASIN.centerZ`: `bz = -cos(th) * 8.6`
  stood them at C.z-8, eight metres out in the meadow beside the centre stream
  instead of at the water's edge, and `world.box` bakes shell geometry with no
  collider, so you walked through them. The lip that replaces them is 52
  instances on the talus mesh atmosphere.js already builds — no draw call, no
  geometry, and it inherits the talus's value.
- **§3 the plunge pool cannot be waded into.** A player driven at the water
  from 24 bearings used to drown on most of them; the worst y reached is now
  −0.44 against a death line of −1.5. The one gap is the bridge lane, barred by
  a low weir that sinks when the FIRST STONE rises — not when the bargain is
  struck, because until something is there to stand on the water is still what
  kills you. Set in `waterfallTaken()` AND in the respawn restore branch, the
  two places the stones and `caveZone` already live.
- **§4 arriving at the cave stops killing you, and the mouth is dressed before
  you can see in.** Two separate things; both below.
- **§5 the freezes.** The whole class, with numbers, below.

## What was not what it looked like

- **The freezes were three bugs, and the plan named only the first.**
  `startGame()` did cancel the warm-up — he clicks faster than the idle
  callback, so on his machine it never ran. Measured on that path: 2149 ms in
  the bedroom, 816 house, 716 basement, 566 forest, 1350 cave, 2399 entering
  the mirror room. Fixing it left 2–5 programs still linking per act at ~600 ms
  each, and the three.js cacheKeys named why: `numPointLights: 29` where the
  warm pass had compiled 32. **`render()` drops the camera to layer 0 for the
  world pass and to LAYER_HELD for the held pass; the warm pass compiled with
  the camera at its RESTING mask**, so every program it built was keyed on a
  light census no frame ever draws. Third: the double in the glass wears a
  clone of the skull, lamps and all, and it was cloned at the door of the
  finale — two point lights joining the mirror census in the last act: 26
  programs, 8.8 seconds, once, at the climax. It is mounted at boot now, inside
  the pinned census, and the reflection warms with the figure standing (compile
  gathers lights with `traverseVisible`, so warming through a hidden double
  compiled the wrong count anyway).
- **The "async" warm path was itself a ten-second freeze.** `compileAsync()`
  links the same programs and then polls `isReady()` every 10 ms; on ANGLE/D3D11
  that poll does not return early. Side by side, same coverage: compileAsync
  gave a 10052 ms frame, a 10.3 s texture pass and an 11.4 s title hold; plain
  `compile()` gives 599 ms, 215 ms and 0.7 s. Only a patient player ever met
  that one — his fast clicks cancelled it and bought the mid-play freezes.
- **Texture ORDER mattered as much as texture warming.** Uploading after the
  compile queues every upload behind the driver's background shader work:
  9954 ms. Before it: 3 ms.
- **A visual acknowledgment applied in the same task as the work never
  paints.** The press must yield two frames before the warm pass starts, or the
  player presses and the tab simply stops for a second and a half.
- **`light.visible = inCave` does NOT move the census** — `World.pinLight`
  intercepts it. Measured per frame across the whole act tour: the visible
  point-light count never moves. The plan's §4(b) suspicion is closed.
- **His "giant untextured surface" at the cave door is the cave's own
  structural rock, seen from outside and undressed.** It is compiled into the
  world's shared batch and therefore always drawn, while the skin that breaks
  it into rock — and the stalactites, the mica, the cataracts — only appeared
  at the act line.
- **The death at the cave door was a floor, not a wall,** and it could not be
  fixed with a collider: `groundHeightAt` reads ramps, rooms and terrain and
  never colliders. The first version added a perfectly good collider that
  blocked nothing and drowned the probe exactly as before.
- **Value near the lantern is a house style, not a bug to fix one prop at a
  time.** The shore lip was built four ways and photographed from the pose a
  player takes two metres away (near-band means: 0x46535d 50.9, 0x232c33 44.7,
  0x0e1318 36.5, unlit 31.1). The darker it got the less it read as rock, and
  the unlit one came out a flat grey cutout. Then the same pose was shot after
  the thaw: the ten bridge stones at that distance are just as bright. Pale
  near the lantern is what rock does in this game.

## The numbers

- A warmed boot walks the entire game and links **ZERO** new shader programs,
  with **zero** frames over 150 ms. Every act enters in 25–115 ms (was
  550–2400). Press-to-play on a machine that never goes idle: **1.2 s**, all of
  it on the title behind an acknowledged press; on a title he lets settle,
  1.6 ms.
- Gates: smoke, autotest 26, regressions 157, playthrough COMPLETE, shore 14,
  warm-start 18, perf-pool, district-culling 15 (max **438**/450, clearing 213,
  cave 201, mouth 231), render-perf.
- New suites: `tests/warm-start-regression.mjs`, `tests/basin-shore-regression.mjs`.
- New tools: `probe-cold-start`, `probe-first-draw`, `probe-lazy-materials`,
  `probe-light-census`, `probe-cave-door`, `shot-shore`. `?hitch=1` logs every
  frame over 150 ms with its program, geometry and texture deltas;
  `__FETCH.warm()` reports the whole warm state including press-to-play.

## ALSO SEEN, NOT FIXED

- **`underfalls-expansion` fails TWO checks**, not one: the mirror-room
  reflected-body check (known) and "the broken promise still gates the expanded
  district". Both were run against the untouched round-four tree in the same
  session and fail identically there — pre-existing, and now recorded as two.
- The graveyard still costs ~1130 draws looking SOUTH against a 450 ceiling
  (mid-yard 1133 S / 298 N). `RECORDED, NOT ASSERTED` in district-culling.
- Tree trunks near the falls' far streams clip to white under the lantern; the
  west mausoleum interior still reads as a bright box (aug14 list).

## Round six is his: "make stuff look good"

Do not start it from these notes — the carry-forwards above are a queue, not
the brief. Wait for his.
